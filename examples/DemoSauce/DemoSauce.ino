...
//#include "tower.h" //Frank's "TeensyTower" Hardware
#include "sound.h"
...
void loop() {
  ...
  for( uint_fast8_t s=0; s<SAMPLES_PER_FRAME; s++ ) {
    uint_fast16_t sample = rms1.read()*511;
    frameParams.audioPeak = max( frameParams.audioPeak, sample );
    sum += sample;
  }

  
  frameParams.audioMean = sum * (1.0 / (511*SAMPLES_PER_FRAME));  // Range: 0..1
